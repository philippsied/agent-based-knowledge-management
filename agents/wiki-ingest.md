...
- **Parallel ingest sub-agents MUST NOT call `scripts/allocate-address.py` directly.** The allocator is flock-guarded for atomicity, but the `.raw/.manifest.json` `address_map` update pattern assumes single-writer semantics.
