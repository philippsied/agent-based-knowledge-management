...
Slide decks MUST look visually distinct in dark vs light themes. Gradient backgrounds must change per theme:
...
### Slide Deck Chart Container (CRITICAL)

Chart slides in presentations MUST follow the same container standards as dashboards, using the `chart-container` class for evaluation consistency, with a **minimum height 400px** (larger than dashboards for presentation readability) and `maintainAspectRatio: false`:
