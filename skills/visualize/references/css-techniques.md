...
## Single-Screen / Fixed-Dimension Posters

For "one screen," "phone screen," "9:16," or "mobile-fit" requests, build a **fixed-dimension single-viewport** poster — NOT a scrolling page. Every pixel is intentional; `overflow: hidden` is what makes it "one screen."

**Dimensions:**
- **9:16 portrait (phone):** `width: 1080px; height: 1920px;` — Instagram Story / phone screen
...
- **`flex: 1` on the main content area** (grid, body) so it expands to fill ALL remaining space between header and footer. Never use fixed `height` values that leave dead space.
- Wrap each logical section in a `<div>` so flexbox distributes them as blocks.
- **Zero dead space rule:** the canvas should be 100% utilized. If there's visible empty space, expand content or reduce padding.
- **No hamburger menu** for fixed-dimension posters — it wastes space; the poster is for screenshot/export, not interaction.

**Content density for 9:16:** hero (title+subtitle) ~25% height, 2-3 content sections ~55%, footer/CTA ~10%, breathing room ~10%. If it looks empty, content is too small — scale up fonts, add grid items, use larger icons.

**Font sizing for 1080px-wide posters:** hero h1 `68-80px`; section labels `15-18px` uppercase (letter-spacing `0.06em`); card text `16-20px`; body `20-24px`.

**Common mistake:** making a scrolling page and screenshotting it — that's a webpage screenshot, not a poster.

## Slide-Container Responsive Breakpoints

Use `clamp()` and container queries for mobile-friendly 16:9 slides:
...
True component-level responsiveness beyond viewport media queries:
...
## Mobile-First Responsive Grid (MANDATORY)
